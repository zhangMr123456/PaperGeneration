...
from models.utils.model import clean_json_for_model


class CleanPydanticOutputParser(JsonOutputParser, Generic[TBaseModel]):
    ...
    def _parse_obj(self, obj: dict) -> TBaseModel:
        try:
            cleaned = clean_json_for_model(self.pydantic_object, obj)
            for patch in cleaned.patches:
                print(f"解析失败: {patch}")
            if issubclass(self.pydantic_object, pydantic.BaseModel):
                return self.pydantic_object.model_validate(cleaned.cleaned)
            if issubclass(self.pydantic_object, pydantic.v1.BaseModel):
                return self.pydantic_object.parse_obj(cleaned.cleaned)
