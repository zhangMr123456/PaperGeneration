...
def parse_folder(folder_path, output_dir, config_path, task=None, split_pages=False, group_size=None,
    ...
    if group_size and group_size > 1:
        ...
        for i, file_group in enumerate(file_groups, 1):
            ...
            try:
                if task:
                    result_dir = single_task_recognition_multi_file_group(file_group, output_dir, MonkeyOCR_model, task,
                                                                          folder_path)
                else:
                    result_dir = parse_multi_file_group(file_group, output_dir, MonkeyOCR_model, folder_path,
                                                        split_pages, pred_abandon)
    ...
    for file_idx, (file_infer_result, file_path) in enumerate(zip(infer_result, file_paths)):
        ...
        if isinstance(file_infer_result, list):
            ...
            for page_idx, page_infer_result in enumerate(file_infer_result):
                ...
                page_pipe_result.dump_content_list(page_md_writer, f"{file_name}_page_{page_idx}_content_list.json",
                                                   page_image_dir)
...
def parse_file(input_file, output_dir, MonkeyOCR_model, split_pages=False, pred_abandon=False):
    ...
    if isinstance(infer_result, list):
        ...
        for page_idx, page_infer_result in enumerate(infer_result):
            ...
            # Save page-specific results
            page_infer_result.draw_model(
                os.path.join(page_local_md_dir, f"{name_without_suff}_page_{page_idx}_model.pdf"))
            page_pipe_result.draw_layout(
                os.path.join(page_local_md_dir, f"{name_without_suff}_page_{page_idx}_layout.pdf"))
            page_pipe_result.draw_span(
                os.path.join(page_local_md_dir, f"{name_without_suff}_page_{page_idx}_spans.pdf"))
            page_pipe_result.dump_md(page_md_writer, f"{name_without_suff}_page_{page_idx}.md", page_image_dir)
            page_pipe_result.dump_content_list(page_md_writer, f"{name_without_suff}_page_{page_idx}_content_list.json",
                                               page_image_dir)
